...
void mvp_a_ui_debug_create(lv_obj_t *parent, const mvp_a_ui_page_t *page)
{
    ...
    snprintf(detail, sizeof(detail), "Stage %u  Diary 0x%x", pet.stage, pet.diary_flags);
    mvp_a_lvgl_page_create(parent, page, detail, &ctx);
    mvp_a_lvgl_page_set_visual_text(&ctx, "Debug",
                                    mvp_a_debug_get_action_name(mvp_a_debug_get_selected()));
}
