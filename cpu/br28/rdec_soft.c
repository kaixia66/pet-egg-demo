...
static void timer_hw_cb(int tid)
{
    ...
    case RDEC_PHASE_1:
        if (timer_mode == TIMER_TIMEOUT) {
            ...
            } else if (timeout_cnt < (500000 / rdec_cfg_info[tid]->timer.filter)) {
                if (last_edge) {
                    if (!gpio_read(rdec_b)) {
                        timer_set_work_mode(tid, CAPTURE_EDGE_RISE);
                        rdec_cnt += rdec_dir;
            ...
            last_edge = 0;
            timeout_cnt = 0;
            /* gptimer_set_resolution(tid, rdec_cfg_info[tid]->timer.filter); */
...
void rdec_soft_test_func()
{
    ...
    rdec_soft_cfg.rdec.rdec_a  = IO_PORTA_13; //编码器A相
    rdec_soft_cfg.rdec.rdec_b  = IO_PORTA_14; //编码器B相
    rdec_soft_cfg.rdec.cnt  = 0; //初始化清0
    rdec_soft_cfg.rdec.mode = RDEC_PHASE_1; //编码器模式
