...
extern struct audio_encoder_task *encode_task_1;
...
#if TCFG_USER_EMITTER_ENABLE && TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
    if (encode_task_1) {
        return -1;
    }
#else
...
#if TCFG_USER_EMITTER_ENABLE && TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
    extern int audio_encoder_task1_open(void);
    audio_encoder_task1_open();
#else
...
#if TCFG_USER_EMITTER_ENABLE && TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
    audio_encoder_open(&sbc_enc->encoder, &sbc_enc_input, encode_task_1);
#else
...
#if TCFG_USER_EMITTER_ENABLE && TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
    extern void audio_encoder_task1_close(void);
    audio_encoder_task1_close();
#else
...
int a2dp_sbc_encoder_init(void *sbc_struct)
{
    ...
    extern void audio_bt_emitter_change_freq(int sr);
    if (sbc_struct) {
        ...
        if ((bt_phone_dec_is_running() || call_ctrl_get_status() == BT_CALL_ACTIVE) && (get_emitter_connect_status() != BT_STATUS_WAITINT_CONN)) {          // 如果连接着耳机，则改sbc编码采样率，以适配msbc
            sbc_param.frequency = SBC_FREQ_16000;
            audio_bt_emitter_change_freq(16000);
        }
#endif
        ...
        audio_bt_emitter_change_freq(44100);
#endif
