...
struct audio_encoder_task *encode_task_1 = NULL;
#endif
...
#if TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
static struct __encoder_task enc_task_1 = {0};
int audio_encoder_task1_open(void)
{
    local_irq_disable();
    if (enc_task_1.init_ok == 0) {
        atomic_set(&enc_task_1.used, 0);
        os_mutex_create(&enc_task_1.mutex);
        enc_task_1.init_ok = 1;
    }
    local_irq_enable();

    os_mutex_pend(&enc_task_1.mutex, 0);
    if (!encode_task_1) {
        encode_task_1 = zalloc(sizeof(*encode_task_1));
        audio_encoder_task_create(encode_task_1, "audio_enc_1");
    }
    atomic_inc_return(&enc_task_1.used);
    os_mutex_post(&enc_task_1.mutex);
    return 0;
}

void audio_encoder_task1_close(void)
{
    os_mutex_pend(&enc_task_1.mutex, 0);
    if (encode_task_1) {
        if (atomic_dec_and_test(&enc_task_1.used)) {
            audio_encoder_task_del(encode_task_1);
            //local_irq_disable();
            free(encode_task_1);
            encode_task_1 = NULL;
            //local_irq_enable();
