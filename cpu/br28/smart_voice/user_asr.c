...
#if ((TCFG_SMART_VOICE_ENABLE) && (TCFG_AUDIO_ASR_DEVELOP == ASR_CFG_USER_DEFINED))
...
/*
*********************************************************************
*       user_asr audio_smart_voice_core_handler
...
int audio_smart_voice_core_handler(void *priv, int taskq_type, int *msg)
{
    ...
    case SMART_VOICE_MSG_MIC_OPEN: /*语音识别打开 - MIC打开，算法引擎打开*/
        /* msg[1] - MIC数据源，msg[2] - mic的数据总缓冲长度，msg[3] - 音频采样率*/
...
int audio_smart_voice_detect_open(u8 value)
{
     ...
     *
     */
    smart_voice_core_post_msg(4, SMART_VOICE_MSG_MIC_OPEN, VOICE_MCU_MIC, 4096, 16000);
...
int audio_smart_voice_detect_init(struct vad_mic_platform_data *mic_data)
{
    ...
    return audio_smart_voice_detect_open(NULL);
}
