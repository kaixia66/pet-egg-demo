...
static int need_trim_flag = 0;

__attribute__((noinline))
AT_RAM
static u8 wiovdd_trim(u8 trim, s16 *adc_wiovdd_voltage, u32 check_vbat)
{
    u32 wiovdd_lev = 7;
    u32 miovdd_lev = 0;
    ...
    if (trim) {
        for (; wiovdd_lev > 3; wiovdd_lev--) {
            ...
            udelay(250);
            /** 判断电压**/
            u32 check_cur_vbat = adc_get_voltage_preemption(AD_CH_VBAT) * 4;
            if (__builtin_abs(check_cur_vbat - check_vbat) > 150) {
                return 1;
            }
...
__attribute__((noinline))
AT_RAM
static u8 miovdd_trim(u8 trim, s16 *adc_miovdd_voltage, u32 check_vbat)
{
    u32 wiovdd_lev = 0;
    u32 miovdd_lev = 7;
    P33_CON_SET(P3_ANA_CON5, 0, 3, miovdd_lev);
...
{
    const u32 m_iovdd_lev = P3_ANA_CON5 & 0x7;
    const u32 w_iovdd_lev = P3_ANA_CON5 & (0x7 << 3);
    if (trim == 0) {
        goto exit_vddio_trim;
    }
    ...
    u8 ret_miovdd_trim = miovdd_trim(trim, adc_miovdd_voltage, check_vbat);
    u8 ret_wiovdd_trim = wiovdd_trim(trim, adc_wiovdd_voltage, check_vbat);

    P33_CON_SET(P3_ANA_CON5, 0, 6, m_iovdd_lev | w_iovdd_lev);
    if (lvd_con & BIT(0)) {
    ...
    if (ret_wiovdd_trim || ret_miovdd_trim) {
        goto exit_vddio_trim;
    }
    ...
    *miovdd_lev = mlev;
    if (__builtin_abs(mlev - m_iovdd_lev) > 1) {
        goto exit_vddio_trim;
    }
...
exit_vddio_trim:
    ...
    *wiovdd_lev = (w_iovdd_lev >> 3) | BIT(7);
    return 0;
}
...
static u8 pvdd_trim(u8 trim)
{
    ...
    if (lev_high_now > PVDD_LEVEL_MAX) {
        lev_high_now = PVDD_LEVEL_MAX;
    }
...
void volatage_trim_init()
{
    ...
    if (vm_need_recover()) { // 升级完后重新trim
        /*破坏原vm数据 用于ota后上电重新trim识别*/
        u32 vm_need_recover_magic = 0xAABBCCDD;
        syscfg_write(VM_PMU_VOLTAGE, (u8 *)&vm_need_recover_magic, sizeof(u32));
    }

    trim = load_pmu_trim_value_from_vm();

    printf("trim = 0x%x\n", trim);
    /* trim wvdd */
    u8 wvdd_lev = wvdd_trim((trim & TRIM_WVDD) ? 1 : 0);
    u8 pvdd_lev = pvdd_trim((trim & TRIM_PVDD) ? 1 : 0);
    ...
    if (trim & TRIM_IOVDD) {
        trim_succ = iovdd_trim(trim, &miovdd_lev, &wiovdd_lev);
    } else {
    ...
    if (trim) {
        if (trim_succ && (wvdd_lev != WVDD_LEVEL_ERR) && (pvdd_lev != PVDD_LEVEL_ERR)) {
    ...
    printf("miovdd_lev = %x\n", miovdd_lev);
    printf("wiovdd_lev = %x\n", wiovdd_lev);
    printf("need_trim_flag:%x\n", need_trim_flag);

    P33_CON_SET(P3_ANA_CON5, 0, 3, miovdd_lev & 0x7f);
    P33_CON_SET(P3_ANA_CON5, 3, 3, 0);
}

int get_need_trim_flag()
{
    return need_trim_flag;
}
