...
/* ------------------------------------------------------------------------------------*/
/**
 * @brief lcd_te_wait 外部自定义TE同步等待函数，
 * 用于在等待TE时将CPU释放出去，避免因UI任务优先级过高导致其它任务卡住
 */
/* ------------------------------------------------------------------------------------*/
static void lcd_te_wait()
{
    ...
    /* 否则启动 TE 中断，使用信号量等待TE，释放CPU */
    power_wakeup_index_enable(5, true);

    /* 等待 TE 信号量 */
#if 0
    /* 如果不考虑超时，可pend 0，直到信号量过来 */
    os_sem_pend(&te_sem, 0);
#else
    /* 考虑超时时，屏幕可能工作异常，允许超时重新初始化 LCD */
    int sem_err = OS_TIMEOUT;
    int te_sem_count = 0;

    while (sem_err) {
        sem_err = os_sem_pend(&te_sem, 1);	// 每次轮询 10ms
        /* printf("@@@ %s, time: %d\n", __func__, te_sem_count); */

        if (++te_sem_count > 50) {	// 总共等待 50 * 10 ms
            printf("Error, waite te timeout...\n");
            /* TE timeout, TODO */
        }
    }
#endif

    /* 关闭 TE 中断，防止某些页面合成时间太久，导致大量中断出现异常 */
    power_wakeup_index_enable(5, false);
}
 ...
 */
/* ------------------------------------------------------------------------------------*/
static void te_signal_sync_int_handler(u8 index, u8 gpio)
{
...
{
    /* 创建 te_sem 信号量 */
    os_sem_create(&te_sem, 0);

    /* 序号需要和板级配置中的wk_param对应上，TE 中断号等于5 */
    port_edge_wkup_set_callback_by_index(5, te_signal_sync_int_handler);

    /* 注册TE等待回调 */
    lcd_set_te_wait_cb(lcd_te_wait);
}
...
static void lcd_rgb_buf_malloc()
{
    int lcd_w;
    int lcd_h;
    int lcd_stride;
...
/*
 * 弱函数重定义(连续帧双buffer情况下，rgb接口切换buffer)
 * */
AT_ST77903_RAM
void *lcd_rgb_get_buffer()
{
    return PHY_ADDR(__lcd->lcd_dbuf[__lcd->lcd_dbuf_index]);
}
...
int lcd_drv_init(void *p)
{
    ...
    /* 如果有使能IO，设置使能IO输出高电平 */
    lcd_en_ctrl(true);
#if TCFG_LCD_CLK_STRENGTH_ENABLE
    gpio_set_hd0(TCFG_LCD_CLK_STRENGTH_PIN, TCFG_LCD_CLK_STRENGTH_LEVEL & BIT(0));
    gpio_set_hd(TCFG_LCD_CLK_STRENGTH_PIN, TCFG_LCD_CLK_STRENGTH_LEVEL & BIT(1));
#endif
    ...
    if (lcd_qspi_st77903_mode()) {
        ...
        lcd_rgb_buf_malloc();
        lcd_draw_area(0, PHY_ADDR(__lcd->lcd_dbuf[__lcd->lcd_dbuf_index]), 0, 0, __this->lcd_width, __this->lcd_height, 0);
    }
...
static void lcd_rgb_start(void)
{
    if (lcd_rgb_mode()) {
