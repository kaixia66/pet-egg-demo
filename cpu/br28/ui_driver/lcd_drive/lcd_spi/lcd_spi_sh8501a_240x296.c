...
注意：这是为br35准备的240x296屏，TP驱动对应CST816D！！！
...
** 初始化代码
*/
static const u8 lcd_spi_sh8501a_cmd_list_poweron[] ALIGNED(4) = {
#if 0
    /* 屏厂给的驱动，仅用作屏显测试 */
    _BEGIN_, 0x11, _END_,
    _BEGIN_, REGFLAG_DELAY, 20, _END_,
    _BEGIN_, 0xc0, 0x5a, 0x5a, _END_,
    _BEGIN_, 0xba, 0x81, _END_,
    _BEGIN_, 0xc1, 0x5a, 0x5a, _END_,
    _BEGIN_, 0x2a, 0x00, 0x00, 0x00, 0xef, _END_,
    _BEGIN_, 0x2b, 0x00, 0x00, 0x01, 0x27, _END_,
    _BEGIN_, 0x44, 0x01, 0x27,  _END_,
...
*/
static int lcd_spi_sh8501a_backlight_ctrl(u8 percent)
{
    if (percent) {
...
** lcd电源控制
*/
static int lcd_spi_sh8501a_power_ctrl(u8 onoff)
{
    lcd_en_ctrl(onoff);
    return 0;
}
...
** 设置lcd进入睡眠
*/
static void lcd_spi_sh8501a_entersleep(void)
{
    u8 dstb = 0x01;
...
** 设置lcd退出睡眠
*/
static void lcd_spi_sh8501a_exitsleep(void)
{
#if DEEP_STANDBY
    struct lcd_platform_data *lcd_dat = lcd_get_platform_data();
    ...
    extern struct imd_param lcd_spi_sh8501a_param;
    lcd_init(&lcd_spi_sh8501a_param);
...
static u32 lcd_spi_sh8501a_read_id()
{
    u8 id[3];
...
struct imd_param lcd_spi_sh8501a_param = {
    .scr_x    = SCR_X,
    .scr_y	  = SCR_Y,
    .scr_w	  = SCR_W,
    .scr_h	  = SCR_H,
...
REGISTER_LCD_DEVICE(sh8501) = {
    .logo = "sh8501",
    .row_addr_align    = 2,
    .column_addr_align = 2,

    .lcd_cmd = (void *) &lcd_spi_sh8501a_cmd_list_poweron,
    .cmd_cnt = sizeof(lcd_spi_sh8501a_cmd_list_poweron) / sizeof(lcd_spi_sh8501a_cmd_list_poweron[0]),
    .param   = (void *) &lcd_spi_sh8501a_param,

    .reset = NULL,	// 没有特殊的复位操作，用内部普通复位函数即可
    .backlight_ctrl = lcd_spi_sh8501a_backlight_ctrl,
    .power_ctrl = lcd_spi_sh8501a_power_ctrl,
    .entersleep = lcd_spi_sh8501a_entersleep,
    .exitsleep = lcd_spi_sh8501a_exitsleep,
    .read_id = lcd_spi_sh8501a_read_id,
    .lcd_id = 0x000000,
};
