...
RESFILE *select_resfile(u8 index);
...
#define imb_create_image(file_info) \
        ...
        if (imb_tsk_info.rotate_en) { \
            ...
            if(UI_DATA_STORE_IN_NORFLASH == 0) { \
                ...
                res_cache(imb_tsk_info.file, file.offset, file.len); \
            } \
        } else { \
            if(UI_DATA_STORE_IN_NORFLASH) { \
                ...
                imb_tsk_info.file_len    = file.len; \
                imb_tsk_info.file   = fp ? fp : ui_load_res_by_pj_id(dc->prj); \
                imb_tsk_info.cb             = imb_enter_cb; \
                imb_tsk_info.outcb          = imb_exit_cb; \
            } \
        ...
        if (imb_tsk_info.in_format == PIXEL_FMT_JPEG) { \
            if(UI_DATA_STORE_IN_NORFLASH) { \
                ...
                imb_tsk_info.file_len = file.len; \
                imb_tsk_info.file   = fp ? fp : ui_load_res_by_pj_id(dc->prj); \
                imb_tsk_info.jpg_ginfo_cb = platform_jpg_ginfo; \
        ...
        } else if (imb_tsk_info.in_format == PIXEL_FMT_GIF) { \
            ...
            if (dc->custom_color_en) { \
                imb_tsk_info.text_color = dc->color_rgb565; \
            } else { \
                imb_tsk_info.text_color = -1; \
            } \
        } \
        \
        if ((imb_tsk_info.data_src == DATA_SRC_SRAM) || (imb_tsk_info.data_src == DATA_SRC_PSRAM)) { \
            ...
            if ((p) && ((p->id != imb_tsk_info.id) || (p->src_w != file.width) || (p->src_h != file.height) || ((file.format != PIXEL_FMT_GIF) && (p->cur_format != file.format)))) { \
                imb_task_file_info_release(p); \
                imb_task_lut_tab_release(p); \
            } \
            if (file.format == PIXEL_FMT_GIF) { \
...
static void font_new_cb(void *priv)
{
    ...
    if (get_rect_cover(dst_r, &font_r, &r0) && get_rect_cover(&r0, &text_r, &r)) {
        int i, j;
        struct rect tr;
        memcpy(&tr, &r, sizeof(struct rect));
        get_rect_cover(&tr, &font_priv->draw, &r);
        ...
        u32 image_bit;
        u8 alpha, pixel;
        u32 image_byte_idx, image_bit_idx;
...
#define imb_create_new_cb_text(_width, _height) \
        ...
        memcpy(&text->font_priv.draw, &imb_tsk_info.draw, sizeof(struct rect)); \
...
void __attribute__((weak)) virfat_flash_get_dirinfo(void *file_buf, u32 *file_num)
{
    ...
    for (i = 0; i < sizeof(file_name) / sizeof(file_name[0]); i++) {
        if (p) {
            memcpy(p + 12 * i, file_name[i], strlen(file_name[i]));
        }
    }
...
void ftl_nandflash_get_dirinfo(void *file_buf, u32 *file_num, u8 index)
{
#if TCFG_UI_RES_USE_FAT_FTL_ENABLE
    /* printf("%s index:%d file_buf:%x", __func__, index, file_buf); */
    struct __dev *ftl_dev = dev_manager_check_by_logo(TCFG_UI_RES_FAT_FTL_ROOT);
    FILE *file = NULL;
    FS_DIR_INFO dir_info = {0};

    if (!ftl_dev) {
        printf("dev not find !!!logo:%s\n", TCFG_UI_RES_FAT_FTL_ROOT);
        return ;
    }
    ...
    *file_num = total_num;
    ...
    fget_dir_info(file, index + 1, 1, file_buf);
    if (file) {
        fclose(file);
    }
#endif
...
{
#if defined(TCFG_UI_RES_USE_FAT_FTL_ENABLE) && TCFG_UI_RES_USE_FAT_FTL_ENABLE
    u32 i;
    u32	k;
    u32 index;
    u32 file_num;
    ...
    u8 *bgp_fname = NULL;
    u8 fname_len;
    ftl_nandflash_get_dirinfo(NULL, &file_num, 0);


    for (i = 0, k = 0; i < file_num; i++) {
        FS_DIR_INFO dir_info = {0};
        ftl_nandflash_get_dirinfo(&dir_info, &file_num, i);
        /* if (dir_info.dir_type) { */
        /* printf("dir type err"); */
        /* continue; */
        /* } */
        /* if (!dir_info.fn_type) { */
        /* printf("fn type err"); */
        /* put_buf(dir_info.lfn_buf.lfn, dir_info.lfn_buf.lfn_cnt); */
        ...
        if (dir_info.fn_type) {
            int watch_name_len =  watch_name_convert(dir_info.lfn_buf.lfn, dir_info.lfn_buf.lfn_cnt, NULL, 0);
            fname = zalloc(watch_name_len + 1);
            if (!fname) {
                ASSERT(0);
                return ;
            }
            watch_name_convert(dir_info.lfn_buf.lfn, dir_info.lfn_buf.lfn_cnt, fname, 0);
        } else {
            int watch_name_len = 0;
            for (int name_i = 0; name_i < dir_info.lfn_buf.lfn_cnt; name_i++) {
                if (dir_info.lfn_buf.lfn[name_i] == 0x20) {
                    break;
                }
                watch_name_len++;
            }
            fname = zalloc(watch_name_len + 1);
            memcpy(fname, dir_info.lfn_buf.lfn, watch_name_len);
        }
...
int watch_set_init()
{
    ...
    for (i = 0; i < file_num; i++) {
        ...
        fname_len = strlen(fname);
        ASCII_ToLower(fname, fname_len);
        /* printf("%s dirname:%s len:%d", __func__, fname, watch_name_len); */
    ...
    fname_buf = watch_zalloc(file_num * 12);
    if (fname_buf == NULL) {
...
int ui_sty_cache_head(RESFILE *file)
{
    struct ui_file_head head ALIGNED(4);
    struct window_head window ALIGNED(4);

    res_fseek(file, 0, SEEK_SET);
    res_fread(file, &head, sizeof(struct ui_file_head));
    res_cache(file, 0, sizeof(struct ui_file_head) + head.window_num * sizeof(struct window_head));
    return 0;
}


int ui_sty_cache(RESFILE *file, int page)
{
    struct ui_file_head head ALIGNED(4);
    struct window_head window ALIGNED(4);
    printf("ui_page_cache file : 0x%x, page : %d\n", (int)file, page);

    res_fseek(file, sizeof(struct ui_file_head) + sizeof(struct window_head)*page, SEEK_SET);
    res_fread(file, &window, sizeof(struct window_head));
    printf("window offset=0x%x, len=0x%x\n", window.offset, window.len);
    res_cache(file, window.offset, window.len);
    return 0;
}

int ui_sty_nocache(RESFILE *file, int page)
{
    struct ui_file_head head ALIGNED(4);
    struct window_head window ALIGNED(4);
    printf("ui_page_cache file : 0x%x, page : %d\n", (int)file, page);
    ...
    res_fread(file, &window, sizeof(struct window_head));
    printf("window offset=0x%x, len=0x%x\n", window.offset, window.len);
    res_nocache(file, window.offset, window.len);
    return 0;
}
...
int switch_ui_page(u8 page, u8 prj)
{
    ...
    if ((watch_need_load || (last_watch_style != dial_sty.watch_style)) && !page && (prj == 1)) {
        ...
        if (UI_DATA_STORE_IN_NORFLASH) {
            ui_sty_nocache(ui_load_sty_by_pj_id(prj), page);
            ui_res_nocache(ui_load_res_by_pj_id(prj), page);
        } else {
            ui_sty_cache(ui_load_sty_by_pj_id(prj), page);
            ui_res_cache(ui_load_res_by_pj_id(prj), page);
        }
...
{
    int ui_res_cache(RESFILE * file, int page_num);

    if (UI_DATA_STORE_IN_NORFLASH) {
        ui_sty_nocache(ui_load_sty_by_pj_id(dc->prj), dc->page);
        ui_res_nocache(ui_load_res_by_pj_id(dc->prj), dc->page);
    } else {
        ui_res_cache(ui_load_res_by_pj_id(dc->prj), dc->page);
        ui_sty_cache(ui_load_sty_by_pj_id(dc->prj), dc->page);
    }
...
int imb_task_delete_enter_callback(struct imb_task_head *root, struct imb_task *task)
{
    if (task->cb == imb_gif_enter_cb) {
        ...
        gif_decode_uninit(task, "ui", ui_gif_msg_type_get);
    }
...
static void imb_rgb_out_data(u8 type, struct rect *rect, u8 *lcd_buf, int stride, int left, int top, int width, int height, int wait)
{
    lcd_data_copy(type, rect, lcd_buf, stride, left, top, width, height, wait);
}

static void imb_rgb_out_over(u8 buf_index)
{
    lcd_data_copy_wait();
    imb_buffer_unlock(buf_index);
}
...
static int br28_put_draw_context(struct draw_context *dc)
{
    ...
    } else if (lcd_rgb_mode()) {
        imb_set_output_cb(imb_rgb_out_data, imb_rgb_out_over);
    } else {
#if defined(TCFG_LCD_FLIP_ENABLE) && TCFG_LCD_FLIP_ENABLE
...
#endif
    static int page_move = 0;
    static int page_index = 0;
    static int page_offset = 0;
    static float page_scale_end = 0;
    static struct block_info *block_info = NULL;
    if (SCALE_EFFECT_WITHOUT_PSRAM_ENABLE && (dc->draw_state == 1)) {

        struct lcd_interface *lcd_hd = lcd_get_hdl();
        struct lcd_info lcd_info = {0};
        if (lcd_hd->get_screen_info) {
            lcd_hd->get_screen_info(&lcd_info);
        }
        if (dc->scale_mode == 1) {
            if (dc->scale_prior == 1) {
                if (!block_info) {
                    u8 *buffer[4];
                    lcd_buffer_get_buffer(0, &buffer[0]);
                    lcd_buffer_get_buffer(1, &buffer[2]);
                    block_info = imb_scale_init(imb_task_head_get(!dc->index), imb_task_head_get(dc->index), lcd_info.width, lcd_info.height, buffer, lcd_info.buffer_size / lcd_info.buf_num, lcd_info.stride);
                    ASSERT(block_info);
                    if (dc->scale_ratio == 0) {
                        page_scale_end = 1.0;
                    } else if (dc->scale_ratio == 1.0) {
                        page_scale_end = 0;
                    }
                }
                ASSERT(block_info);
                imb_scale_jump(block_info, dc->scale_ratio, 0);
                if (dc->scale_ratio == page_scale_end) {
                    if (block_info) {
                        imb_scale_uninit(block_info);
                        block_info = NULL;
                    }
                }
            }
        } else {
            if ((dc->rect_orig.left <= 0) && (!page_move || (page_index == dc->index))) {
                if (!page_move) {
                    page_move = 1;
                    page_index = dc->index;
                }
                ...
                int use_imb_scale;
                if (abs(dc->rect_orig.left - page_offset) > 5) {
                    use_imb_scale = 0;
                } else {
                    use_imb_scale = 1;
                }

                float ratio_l = 1.0 + dc->rect_orig.left * 0.7 / lcd_info.width;
                float ratio_r = 0.3 - dc->rect_orig.left * 0.7 / lcd_info.width;
                ASSERT(block_info);
                imb_scale_move(block_info, dc->rect_orig.left, ratio_l, ratio_r, use_imb_scale);
                page_offset = dc->rect_orig.left;
                if ((dc->rect_orig.left == 0) || (dc->rect_orig.left == -lcd_info.width)) {
                    page_offset = 0;
                    imb_scale_uninit(block_info);
                    block_info = NULL;
                }
...
static int br28_close_draw_context(struct draw_context *dc)
{
    ...
    int ui_res_nocache(RESFILE * file, int page_num);
    ui_sty_nocache(ui_load_sty_by_pj_id(dc->prj), dc->page);
    ui_res_nocache(ui_load_res_by_pj_id(dc->prj), dc->page);
...
static u8 *platform_jpg_ginfo(void *__fat_info, u32 *len, u32 offset_4k)
{
    ...
    if (offset_4k >= ((info->file_len + 4096 - 1) / 4096)) {
        ...
        *len = info->file_len % 4096;
        RESFILE *fp = ui_load_res_by_pj_id(info->prj);
...
void gif_play(int root_index, struct imb_task *task, int elm_id)
{
    int ret = gif_decode(task);
    if (ret == -1) {
        return;
    }
    ...
    struct rect r;
    lcd_drv_get_info(&lcd_info);
    lcd_draw_area_align(&lcd_info, &task->rect, &r);

    imb_task_redraw(imb_task_head_get(task->root_index), &r, STATE_SYNTHESIS_AND_DRAW);
}
...
static void imb_gif_timer(void *priv)
{
    ...
    ui_gif_play_post(task, jiffies_msec(), gif_info->gif_delay, task->elm_id, task->root_index);
...
static void *imb_gif_enter_cb(void *priv)
{
    ...
    if (gif_info->task) {
        if (gif_info->gif_type == 0) {
            imb_task_enable(gif_info->task, false);
...
static int br28_show_text(struct draw_context *dc, struct ui_text_attrs *text)
{
    ...
    struct image_file gif_image_file = {0};
    struct gif_frame_info gif_frame_info;
    struct imb_task_info imb_tsk_info = {0};
    struct imb_task *p = NULL;
    u8 *pixbuf = NULL;
    int strpic_flag = 0;
...
__strpic_with_text:
            ...
            if ((!p) || (p->id != curr_id) || p->scroll || lang_id_change) {
                ...
                if ((info->flags & FONT_SHOW_SCROLL) && roll) {
                    ...
                    if (text->last_crc != curr_crc || (!p) || lang_id_change) {
                        font_text_out(text->encode, info, (u8 *)(text->str + roll * text->offset), text->strlen - roll * text->offset, x - dc->rect_ref.left, y - dc->rect_ref.top);
                        pixbuf = info->text_image_buf;
                    }
                }
        ...
        for (i = 0; i < strpic_num; i++) {
            ...
            if (has_crop_area(&imb_tsk_info.rect, &dc->lcd_rect, &draw)) {
                get_rect_cover(&imb_tsk_info.rect, &draw, &imb_tsk_info.crop);
            } else {
...
__one_strpic_end:
        ...
        if (strpic_mode == 0) {
            ...
            u16 *pstr = (u16 *)text->str;
            int off = 0;

            strpic_idbuf = text->str;
            while (*pstr != 0) {
                if (open_string_pic(dc->prj, &file, *pstr)) {
                    return 0;
                }
                strpic_tbuf = (u8 *)br28_malloc(file.len);
                if (!strpic_tbuf) {
                ...
                ASSERT(((off + file.len - 2) < sizeof(mulstr_tbuf)), "off %d, flen %d\n", off, file.len);

                memcpy(mulstr_tbuf + off, &strpic_tbuf[2], file.len - 2);
                br28_free(strpic_tbuf);
                strpic_tbuf = NULL;
        ...
        while (*str_p != 0) {
            ...
            for (i = 0; i < strpic_num; i++) {
                ...
                memcpy(&imb_tsk_info.crop, &imb_tsk_info.draw, sizeof(struct rect));
                /* }else{ */
                /* memset(&imb_tsk_info.crop, 0x0, sizeof(struct rect)); */
                /* } */
...
__mul_strpic_end:
        ...
        while ((id != 0x00ff) && (id != 0xffff)) {
            ...
            int page = dc->page;
            RESFILE *fp = ui_load_res_by_pj_id(dc->prj);
...
{
    memcpy((u8 *)screen_radius, (u8 *)&__this->info.radius, sizeof(__this->info.radius));
    memcpy((u8 *)screen_fill_argb, (u8 *)&__this->info.fill_argb, sizeof(__this->info.fill_argb));
    return 0;
}
