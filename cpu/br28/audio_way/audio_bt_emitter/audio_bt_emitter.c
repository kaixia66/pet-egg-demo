...
struct audio_bt_emitter_context *ctx_emitter_cpy = NULL;
void audio_bt_emitter_change_freq(int sr)
{
    if (ctx_emitter_cpy) {
        ctx_emitter_cpy->hw_params.rates = sound_pcm_match_standard_rate(sr);
    }
}
#endif
...
#if TCFG_USER_EMITTER_TRANSMIT_A2DP_EN
    if (!ctx_emitter_cpy) {
        ctx_emitter_cpy = ctx;    // 获取emitter context，以便后续根据a2dp或esco修改采样率
    }
#endif
