...
static u32 iic_baud[IIC_HW_NUM] = {0};
...
int _hw_iic_init_old(hw_iic_dev iic)
{
    int ret;
    u8 id = iic_get_id(iic);
...
int hw_iic_init_ic(hw_iic_dev iic);
...
void hw_iic_uninit(hw_iic_dev iic)
{
    ...
    if (id == 0) {
        ...
        gpio_set_pull_up(scl, 0);
        gpio_set_pull_up(sda, 0);
        gpio_disable_fun_output_port(scl);
        gpio_disable_fun_input_port(PFI_IIC_SCL);
        gpio_disable_fun_output_port(sda);
...
{
    if (iic_baud[0] && (iic_info_role(0) == IIC_MASTER))  {
