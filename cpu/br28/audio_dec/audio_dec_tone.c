...
extern void audio_tone_stream_dac_uninit(void);
extern void audio_tone_stream_dac_init(u32 in_sr);
extern struct audio_stream_entry *audio_tone_stream_dac_get_entry(void);
extern int tone_dac_run_stop(struct audio_data_frame *frame);
...
{
    //提示音不走mixer叠加的时候会执行这个函数
    struct audio_data_frame frame = {0};
    frame.stop = 1;
    tone_dac_run_stop(&frame);
}
...
static int tone_dec_file_app_evt_cb(void *priv, enum audio_dec_app_event event, int *param)
{
    ...
    case AUDIO_DEC_APP_EVENT_DEC_CLOSE:
        ...
        dec->hb_convert = NULL;
#endif
        if (!audio_dec_app_mix_en) { //输出到独立的dac
    ...
    case AUDIO_DEC_APP_EVENT_STREAM_OPEN:
        ...
        if (!audio_dec_app_mix_en) { //输出到独立的dac
            audio_tone_stream_dac_init(file_dec->dec->sample_rate);
            (entries_hdl->entries_addr)[entries_hdl->entries_cnt++]  = audio_tone_stream_dac_get_entry();
        }
...
static int tone_dec_sine_app_evt_cb(void *priv, enum audio_dec_app_event event, int *param)
{
    ...
    case AUDIO_DEC_APP_EVENT_STREAM_OPEN:
        ...
        if (!audio_dec_app_mix_en) { //输出到独立的dac
            audio_tone_stream_dac_init(sine_dec->dec->sample_rate);
            (entries_hdl->entries_addr)[entries_hdl->entries_cnt++]  = audio_tone_stream_dac_get_entry();
        }
        break;
