...
// push到app_core任务里面执行
static int tone_play_func_push_to_app_core(int (*func)(int), int info, int *err, OS_SEM *sem)
{
    int msg[5] = {0};
    msg[0] = (int)func;
    msg[1] = 1 | BIT(8) | BIT(9);
...
static int tone_play_open_with_callback_base(const char **list, u8 follow, u8 preemption, void (*evt_handler)(void *priv, int flag), void *evt_priv, int sync_confirm_time)
{
    if (strcmp(os_current_task(), "app_core")) {
        ...
        int ret_err = tone_play_func_push_to_app_core(tone_play_open_with_task_callback, (int)info, err, sem);
        if (ret_err == 0) {
            ret_err = *err;
        }
...
static int tone_play_stop_with_task_callback(int param)
{
...
int tone_play_stop_by_index(u8 index)
{
    ...
    if (strcmp(os_current_task(), "app_core")) {
        ...
        int ret_err = tone_play_func_push_to_app_core(tone_play_stop_with_task_callback, (int)tone_index[index], err, sem);
        if (ret_err == 0) {
            ret_err = *err;
        }
...
int tone_play_stop_by_path(char *path)
{
    ...
    if (strcmp(os_current_task(), "app_core")) {
        ...
        int ret_err = tone_play_func_push_to_app_core(tone_play_stop_with_task_callback, (int)path, err, sem);
        if (ret_err == 0) {
            ret_err = *err;
        }
...
int tone_play_stop(void)
{
    if (strcmp(os_current_task(), "app_core")) {
        ...
        int ret_err = tone_play_func_push_to_app_core(tone_play_stop_with_task_callback, 0, err, sem);
        if (ret_err == 0) {
            ret_err = *err;
        }
