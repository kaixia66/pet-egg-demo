...
{
    struct file_dec_src *file_src = (struct file_dec_src *)priv;

    if (file_src) {
        struct audio_data_frame frame = {
            .data = data,
            .data_len = len,
            .channel = file_src->nch,
            .sample_rate = file_src->out_sample_rate,
        };

        audio_stream_run(&file_src->entry, &frame);
        return file_src->used_len;
    } else {
        return 0;
    }
...
{
    struct file_dec_src *file_src = container_of(entry, struct file_dec_src, entry);
    if (file_src) {
        file_src->used_len = len;
    }
}

static int file_dec_src_data_handler(struct audio_stream_entry *entry,  struct audio_data_frame *in, struct audio_data_frame *out)
{
    struct file_dec_src *file_src = container_of(entry, struct file_dec_src, entry);
    ...
    if (file_src->src_hdl) {
        int wlen = audio_src_resample_write(file_src->src_hdl, in->data, in->data_len);
...
static void *file_dec_src_open(u32 in_sr, u32 out_sr, u8 ch_num)
{
    struct file_dec_src *file_src = zalloc(sizeof(struct file_dec_src));
    ASSERT(file_src);

    file_src->in_sample_rate = in_sr;
    file_src->out_sample_rate = out_sr;
    file_src->nch = ch_num;

    file_src->src_hdl = zalloc(sizeof(struct audio_src_handle));
    ASSERT(file_src->src_hdl);
    audio_hw_src_open(file_src->src_hdl, file_src->nch, SRC_TYPE_RESAMPLE);
    audio_hw_src_set_rate(file_src->src_hdl, file_src->in_sample_rate, out_sr);
    audio_src_set_output_handler(file_src->src_hdl, file_src, file_dec_src_output_handler);

    file_src->entry.data_handler = file_dec_src_data_handler;
    file_src->entry.data_process_len = file_dec_src_data_process_len;

    printf("======   file_dec_src_open succes =====\n");
    return file_src;
}

static void file_dec_src_close(struct file_dec_src *file_src)
{
    if (file_src) {
        printf("file_dec_src_close");
        if (file_src->src_hdl) {
            audio_stream_del_entry(&file_src->entry);
            audio_hw_src_close(file_src->src_hdl);
            free(file_src->src_hdl);
            file_src->src_hdl = NULL;
        }
        free(file_src);
        file_src = NULL;
    }
...
__open_ok:
    ...
    u32 out_target_sr = TCFG_A2DP_MUSIC_SAMPLE_FRAM;
#else
    u32 out_target_sr = dec->file_dec.sample_rate;
#endif /*TCFG_SMART_VOICE_USE_AEC*/

    if (out_target_sr != dec->file_dec.sample_rate) {
        dec->file_src = file_dec_src_open(dec->file_dec.sample_rate, out_target_sr, dec->file_dec.output_ch_num);
    }
...
#endif
    {
        ...
        if (dec->file_src) {
            entries[entry_cnt++] = &dec->file_src->entry;
        }
...
__err3:
    ...
    if (file_dec->file_src) {
        file_dec_src_close(file_dec->file_src);
        file_dec->file_src = NULL;
    }
