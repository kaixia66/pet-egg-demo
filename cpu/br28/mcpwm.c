...
static struct pwm_platform_data mcpwm_info[4];
...
static void clock_critical_exit(void)
{
    ...
    for (u8 ch = 0; ch < 4; ch++) {
        if (JL_MCPWM->MCPWM_CON0 & BIT(ch + 8)) {
            ...
            mcpwm_set_frequency(ch, mcpwm_info[ch].complementary_en, mcpwm_info[ch].frequency);
        }
    }
}
...
void mcpwm_init(struct pwm_platform_data *arg)
{
    ...
    memcpy(&mcpwm_info[arg->pwm_ch_num], arg, sizeof(struct pwm_platform_data));
}
...
void mcpwm_deinit(struct pwm_platform_data *arg)
{
    ...
    if (arg->h_pin < IO_MAX_NUM) {
        gpio_och_disable_output_signal(arg->h_pin, OUTPUT_CH_SIGNAL_MC_PWM0_H + 2 * arg->pwm_ch_num);
        gpio_set_direction(arg->h_pin, 1);
        gpio_set_die(arg->h_pin, 0);
        gpio_set_dieh(arg->h_pin, 0);
        gpio_set_pull_up(arg->h_pin, 0);
        gpio_set_pull_down(arg->h_pin, 0);
    }
    if (arg->l_pin < IO_MAX_NUM) {
        gpio_och_disable_output_signal(arg->l_pin, OUTPUT_CH_SIGNAL_MC_PWM0_L + 2 * arg->pwm_ch_num);
        gpio_set_direction(arg->l_pin, 1);
        gpio_set_die(arg->l_pin, 0);
        gpio_set_dieh(arg->l_pin, 0);
...
#define MCPWM_CON_TMR_EN    8
#define MCPWM_CON_PWM_EN    0
static u16 mcpwm_con_en;
static PWM_TIMER_REG mcpwm_tmr_reg[4];
static PWM_CH_REG mcpwm_ch_reg[4];
AT_VOLATILE_RAM_CODE_POWER
static void mcpwm_enter_deepsleep(void)
{
    mcpwm_con_en = 0;
    for (u8 ch; ch < 4; ch++) {
        memcpy(&mcpwm_tmr_reg[ch], get_pwm_timer_reg(ch), sizeof(PWM_TIMER_REG));
        memcpy(&mcpwm_ch_reg[ch], get_pwm_ch_reg(ch), sizeof(PWM_CH_REG));
...
{
    for (u8 ch; ch < 4; ch++) {
        memcpy(get_pwm_timer_reg(ch), &mcpwm_tmr_reg[ch], sizeof(PWM_TIMER_REG));
        memcpy(get_pwm_ch_reg(ch), &mcpwm_ch_reg[ch], sizeof(PWM_CH_REG));
